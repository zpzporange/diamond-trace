...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating MiningCompanyOrg Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-mc.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for MiningCompanyOrg..."
    fi

    infoln "Creating CuttingCompanyOrg Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-cc.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for CuttingCompanyOrg..."
    fi

    infoln "Creating GradingLabOrg Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-gl.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for GradingLabOrg..."
    fi

    infoln "Creating JewelryMakerOrg Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-jm.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for JewelryMakerOrg..."
    fi
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for Orderer Org..."
    fi
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.miningcompanyorg.example.com miningcompanyorg
    peer_cert admin Admin@miningcompanyorg.example.com miningcompanyorg

    infoln "Creating CuttingCompanyOrg Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.cuttingcompanyorg.example.com cuttingcompanyorg
    peer_cert admin Admin@cuttingcompanyorg.example.com cuttingcompanyorg

    infoln "Creating GradingLabOrg Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.gradinglaborg.example.com gradinglaborg
    peer_cert admin Admin@gradinglaborg.example.com gradinglaborg

    infoln "Creating JewelryMakerOrg Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.jewelrymakerorg.example.com jewelrymakerorg
    peer_cert admin Admin@jewelrymakerorg.example.com jewelrymakerorg
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/miningcompanyorg/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    createMiningCompanyOrg

    infoln "Creating CuttingCompanyOrg Identities"

    createCuttingCompanyOrg
